...
import { AlertCircle, Download, Share, Clock } from 'lucide-react';
...
export default function SharedPhotoView() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.href = photo.url;
      link.download = 'shared-photo.jpg'; // Default name
  ...
  const handleShare = async () => {
    try {
      // Get the current URL which can be shared again
      ...
      <Card className="overflow-hidden bg-white shadow-lg rounded-lg">
        ...
        <div className="p-6 bg-gray-50 border-t text-center">
          <p className="text-sm text-gray-500">
            Shared via <span className="font-semibold">Your Photo Gallery</span>
          </p>
        </div>
      </Card>
    </div>
